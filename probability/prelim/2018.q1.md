---
title: "2018 Problem 1 — Gaussian Identities, Stein's Lemma, and Moment Recursions"
layout: default
math: true
---
...
> $$
> \limsup_{|x|\to\infty} \frac{|f(x)|}{|x|^k} \le C,
> \qquad
> \limsup_{|x|\to\infty} \frac{|f'(x)|}{|x|^k} \le C
> $$
> (In words: $f$ and $f'$ have at most polynomial growth.)
>
> **(i)** Prove that $E(|f'(Z)|) < \infty$ and $E(|Z f(Z)|) < \infty$.  
>
...
$$
\int_a^b -\phi'(z)\,dz = -\phi(z)\Big|_a^b = \phi(a) - \phi(b).
$$
...
# **(b)(i) Show $E|f'(Z)|<\infty$ and $E|Z f(Z)|<\infty$**

## **Assumptions**
- $f$ and $f'$ continuous.  
- Polynomial growth:
  $$
  |f(x)| \le C(1+|x|^k),\quad |f'(x)| \le C(1+|x|^k).
  $$  
- Gaussian moments finite:
  $$ 
  E|Z|^m < \infty \quad \forall m\ge 0.
  $$
...
$$
|f'(x)| \le A(1+|x|^k) \quad \text{for large } |x|.
$$
...
Continuity on compact sets implies boundedness, so for $|x|\le R$:
$$
|f'(x)| \le M.
$$
...
$$
E|f'(Z)| \le A(1 + E|Z|^k) < \infty.
$$

**Step 5. Apply same reasoning to $E|Z f(Z)|$.**  
Using  
$$
|Z f(Z)| \le B(|Z| + |Z|^{k+1}),
$$
we obtain:
$$
E|Z f(Z)| \le B(E|Z| + E|Z|^{k+1}) < \infty.
$$
...
- Boundary term $f(z)\phi(z)\to 0$ as $|z|\to\infty$.
...
$$
-\int f(z)\phi'(z)\,dz
= -f(z)\phi(z)\Big|_{-\infty}^{\infty} + \int f'(z)\phi(z)\,dz.
$$
