...
- Research: `research/README.md` now maps current threads: agricultural/geospatial modeling, representation learning and intrinsic structure, AI-assisted learning/workflows, and proof tooling. `research/concepts/README.md` now maps concept-note themes.
...
- Checked for literal mojibake/control-character issues with `rg`; no real matches found. Earlier `â€™` output appears to have been a shell display decoding issue, not file corruption.
...
- Add actual blog posts or remove/static-adjust placeholder roadmap cards on `index.html` to avoid implying missing links.
